...
> **状态：V1–V4 已于 v1.6.2 全部真机核验完毕（2026-09-22）**，逐条结论见 `docs/whale-widget-spec.md` §10.3 第 ⑤ 段。**验证结论已敲定阶段三的四条硬约束**；**E1（备份 + 可撤销机制）、E2（单个插件禁用 / 启用）、E3（我的一键隔离）与 E4（known-good 快照 / 命名 / 可配保留份数）已随 v1.7.0 全部落地**（见 §6.2 与 spec §10.3 第 ⑥ / ⑦ / ⑧ / ⑨ 段）。
...
| # | 功能 | 前置 | 状态 |
|---|---|---|---|
| E1 | 备份 + 可撤销机制（`whale-dsh-backup/<时间戳>/`） | 无（是所有写入的前置） | ✅ **模块与单测已完成**：`lib/dsh-backup.js` + `test/dsh-backup.test.mjs`（20 项，E4 扩充至 29 项）。**IPC 接入与设置页卡片未做**，留给 E2 —— 详见 spec §10.3 第 ⑥ 段 |
| E2 | 单个插件禁用 / 启用 | V1、V3、E1 | ✅ **已完成**：`lib/dsh-patch.js` + `test/dsh-patch.test.mjs`（15 项）+ `settings.js` 宿主层 + `services.d.ts` 类型 + 设置页「dsh 插件开关」卡片（含快照列表与两步确认还原）。**写前强制建快照，快照失败即中止写入**；「启用」需重启 dsh（V4 单向热重载）。**未新增 IPC 通道**（D14）。详见 spec §10.3 第 ⑦ 段 |
| E3 | 「我的一键隔离」（**不是**原方案的"安全模式"） | V2、E1 | ✅ **已完成**：`lib/dsh-isolate.js`（候选清单纯函数层）+ `dsh-patch.js#applyBatchDisable`（**两趟法**批量落笔）+ `test/dsh-isolate.test.mjs`（25 项）+ `settings.js` 宿主层 + `services.d.ts` 类型 + 设置页「dsh 一键隔离」卡片。**只禁勾选的候选**（§5.2 红线，不做全禁）；候选数上限 `MAX_BATCH = 100`；组装树读不到时**降级**（`treeError` + 只列 patch 条目，不算整体失败）；**写入前 `dryRun` 把将改动的行列给用户过目 → 两步确认 → 建 `before-isolate` 快照（失败即中止）→ 回读逐条校验**。**未新增 IPC 通道**（D14）。详见 spec §10.3 第 ⑧ 段。**分档改造（第 ⑪ 段那轮）**：用户实测 204 个候选（199 条 dsh 官方框架节点 + 5 条第三方插件）平铺不可用 → 候选改为按 dump 分节来源分三档（`patched` / `third` / `core`），设置页分组渲染且 `core` 默认折叠；**取消候选截断**（`MAX_BATCH = 100` 收窄为「只约束一次写入量」）；顺带修掉 §十二 B9（`selectBatch` 曾把用户勾选项静默截断丢弃，改为先保勾选再截断并如实报 `dropped`）。单测 25 → 33 项。详见 spec §10.3 第 ⑪ 段。**三态改造（第 ⑫ 段那轮）**：`disabled` 曾把「调用方漏传 `disabledOf`」压成 `false`，导致 46 条已禁用插件被谎报成「启用中」（真实 dump 两次对照可复现）→ 改为三态 `true` / `false` / **`undefined`（状态未知）**，界面用警示黄虚线渲染、绝不并进「启用中」；分组轴扩为三轴（来源 / 启用状态 / 来源+状态），`both` 轴用 `'tier:state'` 复合 key 避免折叠表撞车。单测 33 → 34 项 |
| — | **E2 + E3 合并为单卡**（第 ⑫ 段那轮） | E2、E3 | ✅ **已完成**：两卡操作同一份 `cordis.patch.yml` / 同一个 profile / 同一份快照，只是入口不同（单条改 vs 批量勾选），分列会让「候选范围不一致」误导用户（E2 的清单只来自 patch，E3 是「patch ∪ 组装树」）→ 合并为「dsh 插件开关」单卡、单条即时 + 勾选批量两种改法并存。**两个 flash 仍不复用**（逐条开关归 `dshPatchFlash`、批量归 `dshIsolateFlash` —— 共用会在两个入口同时开着时串台）。同轮：卡片顺序调为 `主控 → 诊断 → 转储 → 用量 → 插件开关 → Codex`；两份逐字重复的候选行模板抽成 `dshIsolateSubLevels`（改一处漏一处的真缺陷源）；快照块从「说明与候选之间」移到「候选与计划之后」（快照是事后回滚，不该截断「预览 → 确认」的动作链）；主控卡补折叠（默认展开）；Codex 卡前加 `.group-sep` 归属分隔。详见 spec §10.3 第 ⑫ 段 |
| E4 | known-good 快照 + 回滚（留 N 份） | E1。**快照绝不包含 `.credentials.yaml`** | ✅ **已完成**：`dsh-backup.js` 增 `meta.json`（`name` / `knownGood`，**与 `manifest.json` 分开** —— manifest 是快照内容的指纹，标记是用户的意图，改标记不该动指纹）+ `setMeta` / `retentionKeep`，`pruneSnapshots` 支持 **`keep` 只统计未固定快照**（known-good 与手动命名的**不占配额、不被轮转**）+ `protectPinned:false` 退回纯计数语义；`store.js` 增 `config.dshBackupKeep`（`1`–`200`，默认 20）；`settings.js` 增 `dshBackupSetMeta` / `dshBackupPrune` / `dshBackupSetKeep`，`listDshBackups` 带 `max` / `keepMin` / `keepMax`；设置页快照列表加**改名 / 标记「已知良好」/ 删除 / 保留份数**（`test/dsh-backup.test.mjs` 增至 29 项）。**复查另修掉一个真问题**：`listSnapshots` 曾无条件采信 manifest 里的 `m.dirName`，而它会一路进 `rmSync` —— 手改一个字段就能删掉快照根外的目录、或误删另一份**完好**的快照；改为以磁盘目录名为准，只在「旧名确实指向一个含 manifest 的真实目录」时才采信（覆盖目录被手工改名的场景），并在 `pruneSnapshots` 加一道纯目录名校验。**未新增 IPC 通道**（D14）。详见 spec §10.3 第 ⑨ 段 |
...
| 阶段三 | §6 写入与恢复 | **先过 §6.1 的 V1–V4 硬 gate** | **已完成（已随 v1.7.0 发版）**：gate 全过；**E1 备份机制 / E2 单插件禁用启用 / E3 一键隔离 / E4 known-good 标记与可配保留份数已全部落地** |
