 ...
 * dsh 只读诊断（CommonJS）。
 *
 * 设计依据：docs/whale-widget-spec.md §5 / §11（dsh 只读诊断卡）。核心约束：
 *   1. **只读** —— detect 绝不写入任何文件（写入与恢复由「dsh 插件开关」卡负责）。
 *   2. **单项抛错不算整体失败** —— 每项独立 try/catch，失败只标记该项。
 *   3. **平台稳定优先** —— 按「平台语义依赖从零到有」分梯队：
 *      第一梯队 C2/C3/C4（纯文本 + 行级字符串），第二梯队 C1（包身份指纹），
 *      第三梯队 C5（尽力而为，判不出来就报"无法判定"，绝不给错误结论）。
 *   4. **零新 npm 依赖** —— YAML 只做行级字符串扫描，不 parse。
 *
 * 本模块依赖 util.js（叶子模块）与 dsh.js（只调其已导出的 probePort / snapshot，
 * 不改其导出面）。不 require log.js 之外的宿主能力。
 */
const fs = require('fs')
